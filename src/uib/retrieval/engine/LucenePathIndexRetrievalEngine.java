...
public class LucenePathIndexRetrievalEngine extends AbstractRetrievalEngine {
    ...
    public void indexFilesSemantically(String pathToIndex, JLabel statusBar) {
        if (statusBar != null) {
            statusBar.setText("Creating index from semantic annotations");
        }
...
//                            System.out.println(xmlString);
                    ...
                    //if (statusBar != null) parent.setStatus("Parsing documents for nodes: " + df.format((float) i / numAllDocsPercent));
            ...
            if (dir.fileExists(IndexWriter.WRITE_LOCK_NAME)) {
                ...
                statusBar.setText("Existing write.lock at [" + file.getAbsolutePath() 
                        + "] has been found and removed. This is a likely result of non-gracefully "
                        + "terminated server. Check for index discrepancies!");
               
            }
            //dir.close();
            
            Analyzer analyzer = new StandardAnalyzer(Version.LUCENE_30);
            IndexWriter writer = new IndexWriter(dir, analyzer, create, MaxFieldLength.UNLIMITED);

            if (statusBar != null){ 
                statusBar.setText("Creating index for " + element2document.size() + 
                        " different available nodes");}
...
//                addToDocument(idxDocument, semElement, "//Label/Name", "label", all);
            ...
            File pathFile = new File((indexDir));
            Directory pathDir = FSDirectory.open(pathFile);

            IndexWriter pathIndexWriter = new IndexWriter(pathDir, new GraphAnalyzer(), createPathIndexFlag, MaxFieldLength.UNLIMITED);
